...
const validationSchema = Yup.object().shape({
  ...
  point: Yup.number()
    ...
    .required("Poin Task wajib diisi"),
});
  ...
  const onSubmit = async (values) => {
    try {
      ...
      const response = await axiosConfig.post(
        `${BASE_URL}/Guru/${userData.id}/credit/siswa/${siswaId}/`,
        values
      );
      Toast.fire({
        icon: "success",
        ...
        >
          {({
            ...
            >
              ...
              <FormItem
                label="Tipe"
                error={getErrorValue(errors.tipe, errorMessage?.tipe)}
              ...
              <FormItem
                label="Poin"
                error={getErrorValue(errors.point, errorMessage?.point)}
              ...
              <div className="flex justify-end">
                <button className="px-12 py-4 bg-blue text-white font-bold rounded-xl text-xl">
                  Buat Task
                </button>
              </div>
            </Form>
